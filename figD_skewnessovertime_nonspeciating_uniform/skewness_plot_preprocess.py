...
if __name__ == "__main__":
    ...
    for P in PNORMS:
        ...
        for fileID in file_IDs_unique:
            df_strategies = pd.read_csv("{0}/{1}_strategies_nonspeciating.csv".format(dir, fileID))
            drop_col = ["ID", "alive_at_end"]
            df_strategies.drop(columns=drop_col, inplace=True)
            df_strategies["skewness"] = df_strategies.skew(axis=1)

            df_abundances = pd.read_csv("{0}/{1}_results_nonspeciating.csv".format(dir, fileID), index_col="time", header=0)
        ...
        with open(f'skewnessplot_data_p{P}.pkl', 'wb') as file:
