...
if __name__ == "__main__":
    ...
    X = np.linspace(0, 1, 10000)
    Y1, Y2 = circle(X, 0.5)
    plt.plot(X, Y1, c="black", ls="solid", label="Strongly convex")
    Y1, Y2 = circle(X, 0.9)
    plt.plot(X, Y1, c="black", ls=(0, (6, 2)), label="Mildly convex")
    # plt.plot(X, Y2, c="red")
    Y1, Y2 = circle(X, 1)
    plt.plot(X, Y1, c="black", ls=(0, (3, 2)), label="Linear")
    # plt.plot(X, Y2, c="blue")
    Y1, Y2 = circle(X, 1.1)
    plt.plot(X, Y1, c="black", ls="dotted", label="Mildly concave")
    # plt.plot(X, Y2, c="orange")
    Y1, Y2 = circle(X, 2)
    plt.plot(X, Y1, c="black", ls=(0, (1, 5)), label="Strongly concave")
    # plt.plot(X, Y2, c="purple")
    plt.xlim([-0.05,1.05])
    plt.ylim([-0.05,1.05])
    plt.xlabel("$\sigma_{1j}$")
    ...
    plt.tight_layout()
    # plt.legend()
    plt.savefig("tradeoffs_black_5.png")
    plt.show()
    plt.clf()
