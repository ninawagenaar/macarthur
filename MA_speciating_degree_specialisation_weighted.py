...
class ecosystem:
    ...
    def update_system(self):
        ...
        while self.time < self.speciation_events[self.current_event]:
            ...
            '''save results every indicated timestep, results weighted by biomass'''
            if round(self.time, int(round(np.log10(1/self.DT_TIMESCALE),0))) % self.save_every == 0:
                ...
                self.RESULTS[self.STORAGE_COUNTER, 1] = np.average(self.dist_to_generalist[:self.current_event], weights=self.abundances[:self.current_event])
                self.RESULTS[self.STORAGE_COUNTER, 2] = np.average((self.dist_to_generalist[:self.current_event] - np.average(self.dist_to_generalist[:self.current_event], weights=self.abundances[:self.current_event]))**2, weights=self.abundances[:self.current_event])
                self.RESULTS[self.STORAGE_COUNTER, 3] = np.average(self.diff_max_min[:self.current_event], weights=self.abundances[:self.current_event])
                self.RESULTS[self.STORAGE_COUNTER, 4] = np.average((self.diff_max_min[:self.current_event] - np.average(self.diff_max_min[:self.current_event], weights=self.abundances[:self.current_event]))**2, weights=self.abundances[:self.current_event])
                self.RESULTS[self.STORAGE_COUNTER, 5] = np.average(self.dist_to_gamma[:self.current_event], weights=self.abundances[:self.current_event])
                self.RESULTS[self.STORAGE_COUNTER, 6] = np.average((self.dist_to_gamma[:self.current_event] - np.average(self.dist_to_gamma[:self.current_event], weights=self.abundances[:self.current_event]))**2, weights=self.abundances[:self.current_event])   
                self.RESULTS[self.STORAGE_COUNTER, 7] = np.average(self.skewness[:self.current_event], weights=self.abundances[:self.current_event])
                self.RESULTS[self.STORAGE_COUNTER, 8] = np.average((self.skewness[:self.current_event] - np.average(self.skewness[:self.current_event], weights=self.abundances[:self.current_event]))**2, weights=self.abundances[:self.current_event])
