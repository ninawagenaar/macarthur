...
if __name__ == "__main__":
    ...
    for t in np.arange(timestep,  timestep*(num_subplots+1), timestep):
        loc = int(t/timestep)  
        ax = fig.add_subplot(3, 3, loc, projection='3d')
        ax.zorder
        ...
        ax.plot_surface(x, y, z, color="lightgrey", alpha=0.2, zorder=10000)
        ...
        for ID in range(1000):
            if bool(df_abundances.loc[t, "species" + str(ID)]) and not math.isnan(df_abundances.loc[t, "species" + str(ID)]):
                ...
                ax.scatter(alive[0], alive[1], alive[2], c="grey", zorder=0, alpha=1, s=8)
                ax.zorder
    ...
    plt.gca()
    plt.tight_layout()
    plt.savefig(f"titlepic.png", dpi=1000)
    # plt.show()
    plt.clf()
